...
async def get_screenshots_from_page(url: str, download_path: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(url) as response:
            ...
            print(
                f"Found {screenshots_count} screenshot{'s' if screenshots_count > 1 else ''}"
            )
...
async def download_screenshot(
    screenshot: str, download_path: str, session: aiohttp.ClientSession
):
    print(f"Downloading screenshot {screenshot}")

    async with session.get(
        f"https://steamcommunity.com/sharedfiles/filedetails/?id={screenshot}"
    ) as response:
        screenshot_url = re.findall(
            'https\\://steamuserimages-a.akamaihd.net/ugc/[A-Z0-9/].+?"',
            await response.text(),
        )[0]
